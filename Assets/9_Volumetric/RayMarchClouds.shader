Shader "Holistic9/RayMarchClouds"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float random(float3 value, float3 dotDir)
			{
				...
				random = frac(sin(random) * 1234574.43212);
				return random;
			}
			...
			float noise3d(float3 value)
			{
				...
				float3 interpolationVal = frac(value);
				interpolationVal = smoothstep(0.0, 1.0, interpolationVal);
				...
				for (int z = 0; z <= 1; z++)
				{
					...
					for (int y = 0; y <= 1; y++)
					{
						...
						YValues[y] = lerp(XValues[0], XValues[1], interpolationVal.x);
					}
					ZValues[z] = lerp(YValues[0], YValues[1], interpolationVal.y);
				}

				float noise = -1.0 + 2.0 * lerp(ZValues[0], ZValues[1], interpolationVal.z);
				return noise;
			}

			// blend pixel colours from back to front and fade out
			fixed4 integrate(fixed4 sum, float diffuse, float density, fixed4 bgcol, float t) {
				fixed3 lighting = fixed3(0.9, 0.6, 0.6) * 1.3 + 0.5 * fixed3(0.7, 0.5, 0.3) * diffuse;
				fixed3 colorRGB = lerp(fixed3(1.0, 0.95, 0.8), fixed3(0.65, 0.65, 0.65), density);
				fixed4 col = fixed4(colorRGB.r, colorRGB.g, colorRGB.b, density);
				col.rgb *= lighting;
			...
			// ensures there is a range of transparent and opaque clouds; no hard cutoff at MaxHeight
			...
			float map1(float3 q)
			{
				...
				f = 0.5 * noise3d(q);
				q *= 2;
				// add at smaller values for more blotchy effect
				f += 0.25 * noise3d(q);
				q *= 3.5;
				f += 0.15 * noise3d(q);
				return NOISEPROC(f, p);
			...
			fixed4 raymarch(float3 cameraPos, float3 viewDir, fixed4 bgcol, float depth)
			{
				...
				// ct is the accumulation of steps taken
				float ct = 0;
				...
				// using clamp instead of saturate because col is a fixed4
			...
			{
				float depth = 1;
				// distance between camera and pixel location
				depth *= length(i.view);
				// transparent where there are no clouds

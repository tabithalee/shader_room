...
public class Clouds : MonoBehaviour {
	...
	// the camera frustum to pass into shader
	...
	[ImageEffectOpaque] // opaque because we will be blending the background with the clouds
	void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		if(Material == null || ValueNoiseImage == null)
		{
			// see the same image as last frame
			Graphics.Blit(source, destination);
			return;
		}

		// render clouds on top of what Unity is already seeing
		if(_Cam == null)
		{
			_Cam = GetComponent<Camera>(); // placed this here instead of in start because executeineditmode
		}
